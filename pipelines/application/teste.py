...
prompt = ChatPromptTemplate.from_messages([
    ...
    "- Em títulos ou subtítulos, não acrescente asteriscos ou outros caracteres especiais.\n"
    ...
    "- Caso alguem mande uma mensagem que seja apenas ola,oi, comprimentos no geral a resposta deve ser: \"Olá, sou um assistente jurídico. Como posso ajudar?\".\n\n"
    "- Caso a pergunta seja simples(exemplo: qual o cpf de x pessoa, ou qual o nome de y pessoa): responda de forma breve apontando por onde se baseou\n"
    "- Caso a pergunta não faça sentido com os documentos apresentados, não siga o formato da tarefa e responda com apenas 1 frase apontando isto\n"
    "- Caso a pergunta não seja um cumprimento ou uma pergunta simples, siga o seguinte formato:\n"
    "Formato da tarefa(caso não seja um ola/oi ou uma pergunta simples):\n"
...
def handler(event, context):
    ...
    if not update:
        print("⚠️ Nenhuma mensagem nova ou editada encontrada — ignorando update.")
        return {
            'statusCode': 200,
    ...
    user_name   = update['from'].get('username', 'Desconhecido')
    ...
    try:
        print("*** Enviando reply ao Telegram...")
        resp = sendReply(chat_id, reply_message, message_id)
        print("sendReply response: %s", resp)
    except Exception as e:
        print("Erro ao enviar mensagem pelo Telegram", exc_info=True)
