//gerador de números aleatórios para criação do nome do dashboard
resource "random_id" "dashboard_id" {
  byte_length = 4 //para gerar um ID hexadecimal de 8 caracteres
}


resource "aws_cloudwatch_dashboard" "chatbot_dashboard" {                     //cria um recurso do tipo dashboard no cloudwatch
  dashboard_name = "chatbot-telegram-dashboard-${random_id.dashboard_id.hex}" //da o nome visível no console aws (deve ser único)
